      ...
      if (supabase) {
        ...
        if (error) {
          ...
          if (data.session) {
            const user = {
              id: data.user.id,
              name: (data.user.user_metadata?.name) || formData.name || formData.email,
              email: data.user.email || formData.email,
              role: (data.user.user_metadata?.role) || 'admin',
              tenant_id: (data.user.user_metadata?.tenant_id) || data.user.id,
              tenant_name: (data.user.user_metadata?.tenant_name) || formData.organizationName,
            }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">Sign up for your AI Workflow Portal account</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="organizationName" name="organizationName" type="text" placeholder="Your organization" value={formData.organizationName} onChange={handleChange} required />
            </div>
            <div className="space-y-2">
