...
export function WorkflowCanvasFixed({ workflowId, onBack }: WorkflowCanvasProps){
  ...
  const validateWorkflow = () => {
    ...
    steps.forEach(step=>{
      ...
      if(s.service==='voice' && s.action==='Start Voice Call'){ const cfg:any=s.config||{}; const channel=(cfg.channel||'phone').toLowerCase(); if(!cfg.agentId) errors.push({ stepId:step.id, field:'agentId', message:'Agent ID required', level:'error'}); if(channel!=='phone' && channel!=='web') errors.push({ stepId:step.id, field:'channel', message:'Channel must be phone or web', level:'error'}); if(channel!=='web'){ if(!cfg.toPhoneNumber) errors.push({ stepId:step.id, field:'toPhoneNumber', message:'Recipient number required', level:'error'}); else if(!phone.test(cfg.toPhoneNumber)) errors.push({ stepId:step.id, field:'toPhoneNumber', message:'Recipient number looks invalid', level:'warning'}) } }
    })
                        ...
                        {(selectedStepData as any).service && (selectedStepData as any).action && (
                          <div className="space-y-2">
                            ...
                            <div className="rounded-lg border border-border bg-muted/40 p-3 space-y-3">
                              ...
                              ) : ((selectedStepData as any).service==='voice' && (selectedStepData as any).action==='Start Voice Call') ? (
                                <>
                                  <div className="space-y-1"><Label className="text-xs">Agent ID</Label><Input placeholder="Retell agent ID" className={cn("h-8 text-xs", testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='agentId' && e.level==='error') && 'border-red-500 focus-visible:ring-red-500')} value={((selectedStepData as any).config?.agentId as string)||''} onChange={e=>updateStep(selectedStepData.id,({ config:{ ...(((selectedStepData as any).config||{})), agentId:e.target.value } } as any))} />{testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='agentId' && e.level==='error') && <p className="text-[10px] text-red-600">{testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='agentId' && e.level==='error')?.message}</p>}</div>
                                  <div className="space-y-1"><Label className="text-xs">Channel</Label><div className="flex gap-1">{['phone','web'].map(channel=> (
                                    <Button key={channel} variant={(((selectedStepData as any).config?.channel||'phone')===channel)?'default':'outline'} size="sm" className="text-[10px]" onClick={()=>updateStep(selectedStepData.id,({ config:{ ...(((selectedStepData as any).config||{})), channel } } as any))}>{channel==='phone'?'Phone Call':'Web Call'}</Button>
                                  ))}</div><p className="text-[10px] text-muted-foreground">Phone dials a number, web returns a call token.</p></div>
                                  {(((selectedStepData as any).config?.channel||'phone')!=='web') && (
                                    <>
                                      <div className="space-y-1"><Label className="text-xs">Recipient Number</Label><Input placeholder="+1..." className={cn("h-8 text-xs", testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='toPhoneNumber' && e.level==='error') && 'border-red-500 focus-visible:ring-red-500')} value={((selectedStepData as any).config?.toPhoneNumber as string)||''} onChange={e=>updateStep(selectedStepData.id,({ config:{ ...(((selectedStepData as any).config||{})), toPhoneNumber:e.target.value } } as any))} />{testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='toPhoneNumber' && e.level==='error') && <p className="text-[10px] text-red-600">{testErrors.find(e=>e.stepId===selectedStepData.id && e.field==='toPhoneNumber' && e.level==='error')?.message}</p>}</div>
                                      <div className="space-y-1"><Label className="text-xs">Caller ID (optional)</Label><Input placeholder="+1..." className="h-8 text-xs" value={((selectedStepData as any).config?.fromPhoneNumber as string)||''} onChange={e=>updateStep(selectedStepData.id,({ config:{ ...(((selectedStepData as any).config||{})), fromPhoneNumber:e.target.value } } as any))} /></div>
                                    </>
                                  )}
                                  <div className="space-y-1"><Label className="text-xs">Metadata (optional)</Label><Textarea placeholder='{"leadId":"123"}' className="min-h-[50px] text-xs" value={typeof (selectedStepData as any).config?.metadata === 'string' ? ((selectedStepData as any).config?.metadata as string) : (((selectedStepData as any).config?.metadata) ? JSON.stringify(((selectedStepData as any).config?.metadata), null, 2) : '')} onChange={e=>updateStep(selectedStepData.id,({ config:{ ...(((selectedStepData as any).config||{})), metadata:e.target.value } } as any))} /><p className="text-[10px] text-muted-foreground">Optional JSON payload passed to Retell.</p></div>
                                </>
